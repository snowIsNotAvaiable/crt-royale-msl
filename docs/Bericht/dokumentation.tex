...
\vspace{.5cm}
\subsection{Pass~3 -- Phosphor-Maske (vereinfacht)}
\vspace{.5cm}
Pass~3 multipliziert das vertikale Scanline-Bild mit einer RGB-Aperture-Grille. Damit wird Licht in diskrete Subpixel-Streifen gegateted -- der charakteristische Phosphor-Effekt eines CRT-Bildschirms.

\vspace{.5cm}
\textbf{Mapping zur Slang-Referenz.} Pass~3 in unserer MSL-Pipeline entspricht funktional Slang-Pass~7 (\texttt{crt-royale-scanlines-horizontal-apply-mask.slang}). Slang-Passes 2--6 (\texttt{BLOOM\_APPROX}, \texttt{HALATION\_BLUR} V/H, \texttt{MASK\_RESIZE} V/H) sind in dieser Iteration noch nicht portiert -- ihre Beitr\"age zur Slang-Referenz fallen weg, weil die Default-Parameter (\texttt{halation\_weight}=0, \texttt{PHOSPHOR\_BLOOM\_FAKE} aus, \texttt{mask\_sample\_mode}=1) genau diesen Code-Pfad ergeben.

\vspace{.5cm}
\textbf{Vereinfachungen gegen\"uber dem Slang-Original.} Bewusst weggelassen, mit TODO-Kommentaren im Quellcode markiert:
\begin{itemize}[leftmargin=*, nosep]
    \item Horizontale Beam-Filterung (\texttt{sample\_rgb\_scanline\_horizontal} mit Quilez/Lanczos2): direkt linear gesampelt. \\
    \item Konvergenzversatz \texttt{convergence\_offset\_x\_\{r,g,b\}}: auf 0 fixiert. \\
    \item Halation-Komponente: nicht aktiv (Pass 3+4 der Slang-Pipeline noch nicht portiert). \\
    \item Slot-Mask und Shadow-Mask: erfordern PNG-LUT-Loader; auf Aperture-Grille beschr\"ankt. \\
    \item Lanczos-resized LUT (Slang-Passes 5+6): durch direkte prozedurale Erzeugung ersetzt.
\end{itemize}

\vspace{.5cm}
\textbf{Prozedurale Aperture-Grille.} Statt eine externe LUT-Textur zu laden, erzeugt der Kernel die Maske direkt mathematisch: Jede RGB-Triade ist \texttt{mask\_triad\_size} Output-Pixel breit (Default 3) und enth\"alt je eine R-, G-, B-Subpixel-Spalte. Da das Mask-Mittel etwa $1/3$ ist, multipliziert der Kernel mit \texttt{mask\_amplify} ($\sim 3.0$), um die globale Helligkeit zu erhalten. F\"ur Slot- und Shadow-Mask ist das prozedurale Verfahren nicht geeignet (2D-Pattern); diese kommen mit dem LUT-Loader in einer sp\"ateren Iteration.

\vspace{.5cm}
\begin{lstlisting}[language=Metal, caption={Pass~3 -- Apply-Mask-Kernel (Aperture-Grille, prozedural)}]
...
\vspace{.5cm}
\textbf{Validierung.} Die Headless-Pipeline pr\"uft nach jeder Pass-3-Ausf\"uhrung zwei mask-spezifische Invarianten: (1) X-Hochfrequenzenergie steigt deutlich (Subpixel-Streifen) f\"ur near-solid-Inputs, (2) RGB-Triade ist konsistent -- in jedem $x \bmod 3$-Bucket dominiert dieselbe Farbkomponente in $\geq 80\%$ der Spalten. Visuelle Verifikation erfolgt durch direktes Anschauen der Snapshot-PNGs (Color Bars zeigen pro Bar nur die in der Quelle aktiven Subpixel-Komponenten -- wei\ss e Bar alle drei, rote Bar nur R, etc.).

\vspace{.5cm}
\subsection{Debug-Pipeline}
\vspace{.5cm}
Die schrittweise Validierung der 13-Pass-Pipeline ben\"otigt eine M\"oglichkeit, Zwischen-Ergebnisse einzelner Passes auf den Bildschirm zu bringen. Daf\"ur wurde ein \texttt{debug\_pass\_index}-Uniform eingef\"uhrt sowie ein UI-Picker \texttt{Debug -> Show Pass} mit den Optionen \texttt{Final}, \texttt{Pass 1 (Linearize)}, \texttt{Pass 2 (Scanlines V)}.
...
\vspace{.25cm}
\textbf{Erste \(\Delta E\)-Messung.} Tabelle \ref{tab:dE-first} zeigt die mittlere $\Delta E_{2000}$-Differenz unserer MSL-Pipeline-Outputs gegen die librashader-Referenz f\"ur drei Beispiel-Patterns. Befund:
\begin{itemize}[leftmargin=*, nosep]
    \item \texttt{solid\_black}: $\Delta E = 0.00$ \"uber alle Stages -- beide Pipelines erhalten Schwarz exakt.
    \item \texttt{pass2} (Vertical Scanlines): mittlere $\Delta E = 13$--$19$ auf nicht-trivialen Inputs. Visueller Vergleich der Snapshots zeigt, dass beide Pipelines strukturell korrekt arbeiten (Scanlines pr\"asent, Farben + Triadenausrichtung gleich), aber unsere Beam-Modulation ist \emph{zu weich}: die dunklen L\"ucken zwischen Scanlines sind in unserer Ausgabe deutlich heller als in der Referenz. Das ist der erste konkrete Befund f\"ur die n\"achste Iteration -- entweder Beam-Defaults justieren oder die Contribution-Math gegen die Slang-Quelle neu abgleichen.
    \item \texttt{pass3} und \texttt{final}: mittlere $\Delta E = 30$--$50$. Erwartete Divergenz: pass3 nutzt bei uns eine prozedurale Aperture-Grille, die Referenz lookup-basierte Mask-LUTs; \texttt{final} fehlen uns Slang-Passes 2--6 (Bloom-Approx + Halation V/H + Mask-Resize), 8--10 (Brightpass + Bloom V/H) und 11 (Geometry/AA).
\end{itemize}
...
\begin{tabularx}{\textwidth}{llrrrr}
    \toprule
    \tableheadercolor \thw{Pattern} & \thw{Stage} & \thw{$\overline{\Delta E_{2000}}$} & \thw{P95} & \thw{Max} & \thw{SSIM} \\
    \midrule
    colorbars   & pass2  & 13.60 & 26.72 & 28.98 & 0.930 \\
    colorbars   & pass3  & 32.26 & 86.41 & 89.72 & 0.462 \\
    colorbars   & final  & 41.44 & 90.58 & 101.19 & 0.045 \\
    gradient    & pass2  & 14.21 & 27.60 & 29.47 & 0.471 \\
    gradient    & pass3  & 31.62 & 84.01 & 89.56 & 0.047 \\
    gradient    & final  & 29.21 & 59.05 & 87.87 & 0.198 \\
    solid\_black & pass2 &  0.00 &  0.00 &  0.00 & 1.000 \\
    solid\_black & pass3 &  0.00 &  0.00 &  0.00 & 1.000 \\
    solid\_black & final &  0.00 &  0.00 &  0.00 & 1.000 \\
    \bottomrule
\end{tabularx}
\rowcolors{0}{}{} \vspace{.25cm}
\caption{Erste $\Delta E_{2000}$-Vergleichswerte gegen das Slang-Original (Auszug).}
\label{tab:dE-first}
\end{table}
...
\begin{tabular}{clll}
    ...
    3 & Validierung & Per-Pass-Vergleich mit \texttt{librashader-cli}-Reference & In Arbeit (erste $\Delta E$-Werte vorliegen) \\
...
\begin{tabular}{cll}
    ...
    2  & Vertical Scanlines (Beam)       & \checkmark ~Abgeschlossen \\
    3  & Apply Phosphor Mask (vereinfacht, prozedural) & \checkmark ~Abgeschlossen \\
    4  & Bloom Approx (320$\times$240 Resize)        & Ausstehend \\
    5  & Halation Blur Vertical/Horizontal           & Ausstehend \\
    6  & Mask Resize Vertical (Lanczos)              & Ausstehend \\
    7  & Mask Resize Horizontal (Lanczos)            & Ausstehend \\
