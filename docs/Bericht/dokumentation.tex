...
\vspace{.25cm}
\textbf{Aktueller Stand:} 59 / 59 Sanity-Checks gr\"un (Stand dieser Iteration). Eine Code\"anderung, die einen Pass bricht, schl\"agt typischerweise sofort in mindestens einem dieser Checks zu. Visuelle Verifikation der Scanline-Form ist \"uber das direkte Anschauen der \texttt{tests/outputs/}-PNGs m\"oglich (Pass-2-Snapshots zeigen die charakteristische horizontale Streifenstruktur eines CRT).
...
\begin{tabularx}{\textwidth}{lX}
    ...
    \texttt{crt-royale-msl/tools/compare.py} & Slot f\"ur Pixel-Diff-Skript (geplant) \\
    \bottomrule
\end{tabularx}
\rowcolors{0}{}{} \vspace{.25cm}
...
\subsection{Methodik}
\vspace{.5cm}
Die Validierung erfolgt durch \textbf{visuellen A/B-Vergleich} zwischen dem GLSL-Original \newline (in RetroArch) und der MSL-Portierung (in RetroVisor) -- und zwar \textbf{pro Pass}, nicht nur am Pipeline-Ende:
\vspace{.25cm}
\begin{enumerate}[nosep]
    ...
    \item Per-Pass-Snapshot in RetroVisor \"uber den Debug-Picker + Save Snapshot \\
    \item Korrespondierender Snapshot der gleichen Stufe aus RetroArch \\
    \item Pixel-f\"ur-Pixel-Differenzbild via \texttt{tools/compare.py} berechnen \\
